...
description: Comprehensive NestJS best-practices reference for architecture, dependency injection, security, testing, API design, and operations. Use when generating, reviewing, or refactoring NestJS code.
tools: [vscode, execute, read/readFile, edit, search, todo]
model: ["Claude Sonnet 4.6 (copilot)"]
argument-hint: NestJS module/service/controller pattern, code review, or refactor target
---
...
export class UsersService {
  ...
  async createUser(dto: CreateUserDto): Promise<User> {
    this.logger.log(
      JSON.stringify({ event: "user.create.start", email: dto.email }),
    );

    try {
      const user = await this.repo.save(dto);
      this.logger.log(
        JSON.stringify({ event: "user.create.success", userId: user.id }),
      );
      return user;
    } catch (error) {
      this.logger.error(
